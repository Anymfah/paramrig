...
import type { ModulationValue, ValueSource } from './extended-types'
import { normalizeFontValue } from './font-value'
...
export function normalizeValue(param: ParameterDef, value: unknown): ParamValue {
  ...
  switch (param.kind) {
    ...
    case 'color': return typeof value === 'string' && ((param.allowNone && value === 'none') || /^#[\da-f]{6}([\da-f]{2})?$/i.test(value)) ? value : fallback()
    case 'switch': return typeof value === 'boolean' ? value : fallback()
    case 'select':
      if (param.view === 'font-library') return normalizeFontValue(value) ?? fallback()
      return typeof value === 'string' && param.options.some(o => o.value === value) ? value : fallback()
    case 'preset': return typeof value === 'string' && param.options.some(o => o.value === value) ? value : fallback()
