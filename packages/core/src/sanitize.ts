import type { BezierCurve, InspectorCategory, ParameterDef, ParamGroup, Vec2 } from './types'
...
export function sanitizeParameter(value: unknown, groupIds: Set<string>, allow: { extended?: boolean } = {}): ParameterDef | null {
  ...
  if (kind === 'preset' && Array.isArray(source.options)) {
    const choices = source.options.slice(0, 100).flatMap(item => {
      if (!item || typeof item !== 'object') return []
      const option = item as Record<string, unknown>, value = text(option.value, 60)
      if (!value || !option.values || typeof option.values !== 'object' || Array.isArray(option.values)) return []
      const values = Object.fromEntries(Object.entries(option.values).filter(([key, next]) => key.length <= 60 && (typeof next === 'string' || typeof next === 'boolean' || typeof next === 'number' && Number.isFinite(next))).slice(0, 200)) as Record<string, string | number | boolean>
      return [{ value, label: text(option.label, 60) ?? value, values }]
    })
    return choices.length ? { ...base, kind: 'preset', defaultValue: choices.find(option => option.value === source.defaultValue)?.value ?? choices[0]!.value, options: choices } : null
  }
  if (kind === 'number') {
    ...
    return {
      ...
      ...(text(source.fontParameter, 60) ? { fontParameter: text(source.fontParameter, 60)! } : {}),
  ...
  if (kind === 'color') return { ...base, kind: 'color', defaultValue: source.allowNone === true && source.defaultValue === 'none' ? 'none' : hex(source.defaultValue, '#D4E7E1'), ...(source.alpha === true ? { alpha: true } : {}), ...(source.allowNone === true ? { allowNone: true } : {}) }
  ...
  if (kind === 'select') {
    ...
    return { ...base, kind: 'select', options: list, defaultValue: chosen && list.some((item) => item.value === chosen) ? chosen : fallback,
      ...(['search', 'visual', 'font', 'font-library'].includes(String(source.view)) ? { view: source.view as 'search' | 'visual' | 'font' | 'font-library' } : {}) }
  }
  if (kind === 'text') {
