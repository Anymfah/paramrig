 ...
 */
export function objectMesh(document: SceneDocument, object: SceneObject): MeshData | null {
  ...
  const key = `${object.id}|${JSON.stringify(data)}|${taper ? taper.points.length : 0}`
  ...
  const built = data.kind === 'curve' ? curveMesh(data, taper) : textMesh(data)
