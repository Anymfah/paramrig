...
import { ColorController } from '@/ui/ColorController'
...
import type { ControlResources } from '@/ui/ResourceController'
import { SpatialController,RangeController } from '@/ui/SpatialController'
import { PointsController } from '@/ui/PointsController'
import { RadialController } from '@/ui/RadialController'
import { TextController } from '@/ui/TextController'
import { ChoiceController } from '@/ui/ChoiceController'
import { ResourceController } from '@/ui/ResourceController'
import { CameraController, Gizmo2DController, Gizmo3DController, TextureFrameController } from '@/ui/VisualControllers'
import { Button, IconButton } from '@/ui/Button'
import { IconChevron, IconChevronRight, IconChevronUp, IconCopy, IconGrip, IconPlus, IconTrash } from '@/ui/icons'
...
export type ParameterControlProps=GestureProps & {
...
export function ParameterControl({param,value,onChange,onAction,onPreset,resolveNumber,parameters=[],fonts,registry,resources,animated,driven,...gesture}:ParameterControlProps) {
  const Custom=registry?.resolve(param)
  if(Custom)return <Custom param={param} value={value} onChange={onChange} onAction={onAction} onPreset={onPreset} resolveNumber={resolveNumber} parameters={parameters} fonts={fonts} registry={registry} resources={resources} animated={animated} driven={driven} {...gesture}/>
  ...
  switch(param.kind){
    ...
    case 'resource': return <ResourceController param={param} value={value as ResourceValue|null} onChange={onChange} resources={resources}/>
    ...
    case 'group': return <fieldset className="controller-stack controller-fieldset controller-group"><legend>{param.label}</legend>{param.fields.filter(field=>!field.hidden).map(field=><ParameterControl key={field.id} param={field} value={objectValue(value)[field.id]??field.defaultValue} onChange={next=>onChange({...objectValue(value),[field.id]:next})} onAction={onAction} onPreset={onPreset} resolveNumber={resolveNumber} parameters={parameters} fonts={fonts} registry={registry} resources={resources} animated={animated} driven={driven} {...common}/>)}</fieldset>
    case 'list': return <ListController param={param} value={value as ParamValue[]} onChange={onChange} onAction={onAction} onPreset={onPreset} resolveNumber={resolveNumber} parameters={parameters} fonts={fonts} registry={registry} resources={resources} animated={animated} driven={driven} {...common}/>
...
function ListController({param,value,onChange,onAction,onPreset,resolveNumber,parameters,fonts,registry,resources,animated,driven,...gesture}:GestureProps & {param:Extract<ParameterDef,{kind:'list'}>;value:ParamValue[];onChange:(v:ParamValue[])=>void;onAction?:ParameterControlProps['onAction'];onPreset?:ParameterControlProps['onPreset'];resolveNumber?:ParameterControlProps['resolveNumber'];parameters?:ParameterDef[];fonts?:Font[];registry?:ControlRegistry;resources?:ControlResources;animated?:ParameterControlProps['animated'];driven?:ParameterControlProps['driven']}) {
  const listId=useId()
  ...
  return <fieldset className="controller-stack controller-fieldset controller-list"><legend>{param.label}</legend>{value.map((v,i)=>{const expanded=open===i;const itemLabel=`${param.item.label} ${i+1}`;const bodyId=`${listId}-item-${i}`;return <div className="controller-list-item" data-expanded={expanded||undefined} data-drop={drop?.index===i?(drop.after?'after':'before'):undefined} key={i}
    ...
    {expanded?<div className="controller-list-item__body" id={bodyId}><ParameterControl param={i<shipped.length?withDefault(param.item,shipped[i]!):param.item} value={v} onChange={next=>onChange(value.map((old,j)=>i===j?next:old))} onAction={onAction} onPreset={onPreset} resolveNumber={resolveNumber} parameters={parameters} fonts={fonts} registry={registry} resources={resources} animated={animated} driven={driven} {...gesture}/></div>:null}
