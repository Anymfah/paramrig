...
import { textOutlineRuns } from './outlineRuntime'
import { leafElements } from '@/vector/tree'
import type { VectorDocument, VectorElement, VectorPaint, VectorPoint, VectorFont } from '@/vector/types'
...
export function pdfPages(document: VectorDocument, perFrame: boolean): PdfPage[] {
  ...
  if (perFrame && frames.length > 0) {
    return frames.map((frame) => ({
      ...
      elements: [frame, ...leafElements(document.elements, [frame.id]).filter(element => element.id !== frame.id)],
    }))
  }
  return [{
    name: document.name,
...
export async function exportPdf(
  pages: PdfPage[],
  options: { resolveFont?: FontResolver; fonts?: VectorFont[]; background?: string; rasterise?: (element: VectorElement) => Promise<{ data: Uint8Array; width: number; height: number } | null> } = {},
  ...
  for (const page of pages) {
    const resources: Resources = { fonts: options.fonts ?? [], resolveFont: options.resolveFont, states: new Map(), shadings: [], images: [], writer }
    ...
    for (const element of page.elements) {
      if (!element.visible || element.kind === 'group' || element.kind === 'component' || element.kind === 'instance') continue
      await drawElement(element.kind === 'frame' ? { ...element, kind: 'rectangle' } : element, content, resources, notes, options.rasterise)
    }
    content.push('Q')
...
async function drawElement(
  ...
  // Font contours are already closed curves. Sending them through editable planar-face
  // reconstruction can split fine serifs at coincident anchors, so keep the original curves.
  const textRuns = element.kind === 'text' ? await textOutlineRuns(element, resources.fonts, resources.resolveFont) : null
  if (element.kind === 'text' && !textRuns) { notes.skipped.push(element.name); return }
  const geometry = textRuns ? { fillRuns: textRuns, strokeRuns: textRuns } : localGeometry(element)
  const center = { x: element.x + element.width / 2, y: element.y + element.height / 2 }
  content.push('q')
  if (element.rotation && !textRuns) content.push(rotation(element.rotation, center))
  ...
  if (fill && geometry.fillRuns.length) {
    if (fill.type === 'solid' && fill.color && fill.color !== 'none') {
      ...
      content.push(...pathOperators(geometry.fillRuns), `${pdfNumber(r)} ${pdfNumber(g)} ${pdfNumber(b)} rg`, textRuns ? 'f' : 'f*')
    } else if (fill.type === 'linear' || fill.type === 'radial') {
      // A shading paints through the shape rather than into it: clip, then fill the clip.
      content.push(...pathOperators(geometry.fillRuns), textRuns ? 'W n' : 'W* n', `/${shading(resources, fill, element)} sh`)
    }
  }
