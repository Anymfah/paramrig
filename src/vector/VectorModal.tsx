 ...
 */
export function VectorModal({ label, open, onClose, children }: {
  label: string
  open: boolean
  onClose: () => void
  children: ReactNode
  ...
  return createPortal(
    <div className="vector-modal" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <div ref={panel} className="vector-modal__panel" role="dialog" aria-modal="true" aria-label={label}>
        {children}
      </div>
    </div>,
