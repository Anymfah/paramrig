...
export function ChoiceController({label,options,value,onChange,multiple=false,visual=false,font=false,searchable=false}:{label:string;options:Option[];value:string|string[];onChange:(v:string|string[])=>void;multiple?:boolean;visual?:boolean;font?:boolean;searchable?:boolean}) {
  ...
  return <fieldset className="controller-stack controller-fieldset"><legend>{label}</legend>
    {searchable?<input className="controller-input" type="search" aria-label={`Search ${label}`} placeholder="Search options…" value={query} onChange={e=>setQuery(e.target.value)}/>:null}
    <div className={`choice-controller ${visual?'choice-controller--visual':''}`} role={multiple?'group':'radiogroup'} aria-label={label}>
      {choices.map((o,index)=><button key={o.value} type="button" role={multiple?'checkbox':'radio'} aria-checked={selected.includes(o.value)} tabIndex={multiple||selected.includes(o.value)||(!choices.some(c=>selected.includes(c.value))&&index===0)?0:-1} className="choice-controller__option" onKeyDown={e=>{
        ...
        <span style={font?{fontFamily:o.value}:undefined}>{o.label}</span><span aria-hidden="true">{selected.includes(o.value)?'✓':''}</span></button>)}
    </div>{choices.length===0?<p className="field__hint" role="status">No matching options.</p>:null}
  </fieldset>
}
