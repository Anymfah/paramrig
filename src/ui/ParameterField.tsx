...
export function ParameterField({param,value,onChange,onAction,onPreset,resolveNumber,parameters=[],animated,...gesture}:ParameterFieldProps) {
  ...
  switch(param.kind){
    ...
    case 'palette': return <fieldset className="controller-stack controller-fieldset"><legend>{param.label}</legend>{(value as string[]).map((color,i)=><div className="controller-list-row" key={i}><ColorField label={`Color ${i+1}`} value={color} onChange={next=>onChange((value as string[]).map((v,j)=>i===j?next:v))} {...common}/><Tooltip content={`Remove color ${i+1}`}><IconButton label={`Remove color ${i+1}`} onClick={()=>onChange((value as string[]).filter((_,j)=>j!==i))}><IconTrash/></IconButton></Tooltip></div>)}<div className="controller-actions"><Button size="sm" variant="quiet" disabled={(value as string[]).length>=(param.maxItems??16)} onClick={()=>onChange([...(value as string[]),'#808080'])}>Add color</Button></div></fieldset>
