...
export function ResourceController({param,value,onChange}:{param:Extract<ExtendedParameter,{kind:'resource'}>;value:ResourceValue|null;onChange:(v:ResourceValue|null)=>void}) {
  ...
  return <div className="controller-stack">
    <label className="resource-drop" data-filled={value?'':undefined} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();void choose(e.dataTransfer.files[0])}}>
      ...
      <span className="resource-drop__name">{busy?'Saving locally…':value?.name??'Choose or drop a file'}</span><small>{formats} · up to {param.maxMB??25} MB</small>
    </label>
