...
export function ColorController({param,value,onChange,...gesture}:GestureProps & {param:ColorParam;value:string;onChange:(v:string)=>void}) {
  ...
  if(!param.alpha&&!param.channels)return <ColorField label={param.label} value={hex} defaultValue={defaultHex} allowNone={param.allowNone} onChange={onChange} {...gesture}/>
