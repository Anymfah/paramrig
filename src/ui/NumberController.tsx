...
export function NumberController({param,value,onChange,...gesture}: GestureProps & {param:NumberParam;value:number;onChange:(value:number)=>void}) {
  ...
  return <BaseNumberController param={displayParam} value={value/factor} onChange={next=>onChange(next*factor)} units={units!.map(unit=>({value:unit.value,label:unit.label}))} onUnitChange={setUnitValue} {...gesture}/>
}

function BaseNumberController({param,value,onChange,units,onUnitChange,...gesture}: GestureProps & {param:NumberParam;value:number;onChange:(value:number)=>void;units?:{value:string;label:string}[];onUnitChange?:(unit:string)=>void}) {
  const drag=useControllerGesture(gesture)
  const origin=useRef({x:0,value:0})
  const p={label:param.label,value,min:param.min,max:param.max,step:param.step,unit:param.unit,units,onUnitChange,onChange,...gesture}
  ...
  if(view==='knob'||view==='angle') return <div className="controller-stack">
    ...
    <div className="dial-row"><Tooltip content="Drag up or down · Shift for precision · Double-click to reset"><div className="controller-dial" role="slider" tabIndex={0} aria-label={`${param.label} dial`} aria-valuemin={param.min} aria-valuemax={param.max} aria-valuenow={value} aria-valuetext={`${value}${param.unit??''}`}
