...
export function WebControls({ session, controls, disabled, values = session.document.values }: { session: WebSession; controls: ScopedControl[]; disabled: boolean; values?: Values }) {
  ...
  return <ContextMenuRoot><div inert={disabled}>
    {sections.map(section => <ControlSection key={`${section.group.id}:${section.scope}`} group={section.group} controls={section.controls} session={session} values={values} />)}
  </div></ContextMenuRoot>
}

function ControlSection({ group, controls, session, values }: { group: ParamGroup; controls: ScopedControl[]; session: WebSession; values: Values }) {
  const [open, setOpen] = useState(group.defaultOpen ?? true)
  const panelId = useId()
  const doc = session.document
  const dirty = controls.filter(({ param }) => !valuesEqual(values[param.id]!, doc.sourceValues[param.id]!))
  const sharedScope = controls.every(control => control.scope === controls[0]!.scope) ? controls[0]!.scope : null
  const scope = sharedScope?.toLowerCase() === group.label.toLowerCase() ? '' : sharedScope
  const contextual = group.label === 'Selected element'
  const resetSection = () => {
    ...
    <ContextTarget items={[sectionAction]} label={`${group.label} actions`}>
      <div className="section__head">
        <button type="button" className="section__title" aria-expanded={open} aria-controls={panelId} onClick={() => setOpen(value => !value)}>
          <span>{contextual ? sharedScope === 'Selected element' ? 'Controls' : sharedScope : group.label}{scope && !contextual ? <span className="web-section-scope"> {scope}</span> : null}</span><IconChevron />
        </button>
      </div>
    </ContextTarget>
    <div className="section__panel" id={panelId} inert={!open} aria-hidden={!open}><div>
      {controls.map(({ param, scope: controlScope }) => <ContextTarget key={param.id} label={`${param.label} actions`} items={[
        ...
        />
        {sharedScope === null ? <p className="field__hint web-control-scope">{controlScope}</p> : null}
      </ContextTarget>)}
    </div></div>
  </section>
}
