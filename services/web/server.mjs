...
const GUIDE = `# ParamRig feedback\n\nThe whole of it: the @paramrig/web README, which covers the manifest, the attributes, the connection and this loop. JSON Schema for the three files ships with that package under schemas/ — manifest.schema.json, batch.schema.json, response.schema.json.\n\nRead manifest.json, then the newest of the immutable batches/*.json: a batch restates every difference from the source, and a control present in values but absent from changes did not move. Each batch carries approved values, its source revision, exact targets, comments, vector markup and captures. Drafts are not approved instructions.\n\nApply the changes in the source project — the batch is a request, not a patch. A value behind an adapter usually lives in three places that move together: the initial state, read and restore. Preserve the stable data-paramrig-id and data-paramrig-instance attributes. Then, in manifest.json, set each changed control's defaultValue to the value applied, and change revision to any string not used before: a counter, a date, a short label. Write a new responses/<id>.json atomically (temporary file, then rename), named after its own id.\n\nResponse format: { "version": 1, "id": "unique-id", "projectId": "manifest-id", "batchId": "batch-id", "sourceRevision": "batch-source-revision", "resultRevision": "new-manifest-revision", "createdAt": "ISO timestamp", "summary": "What changed", "tickets": [{ "id": "ticket-id", "status": "implemented", "message": "What changed and how to verify" }] }. Use needs-info to request clarification. A response is a claim, not an approval: say what was done and how to check it, and only the user validates a correction. Do not change batches, draft.json or capture files. Captures marked dom are reconstructions and may differ from screen rendering.\n`
