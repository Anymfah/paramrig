...
- État actuel à connaître : `vectorManifest(document)` dans `src/vector/document.ts` (ligne ~116) produit un `RigManifest` avec `renderer: 'vector'` et `parameters: []` ; `src/rigs/registry.ts` liste ces manifestes avec les exemples ; `src/workspace/WorkspacePage.tsx` (ligne ~61) court-circuite tout manifeste `vector` vers `<VectorEditorPage manifest>` avant de créer la `RigSession`, et `src/workspace/RigPreview.tsx` n'a aucun cas `vector`. Il n'existe pas de dossier `src/renderers/vector`.
- Cible : `vectorManifest` expose `groups`, `parameters` et `inspectorCategories` du rig. `WorkspacePage` ne court-circuite plus : un document vectoriel **sans** `rig` ouvre l'éditeur comme aujourd'hui ; un document **avec** `rig` ouvre l'éditeur par défaut, et le mode Tune (chantier N3) rend le `WorkspaceShell` standard avec `Inspector` (`src/workspace/Inspector.tsx`, props `session, categories, groups, parameters, values, defaults`) et un nouveau `src/renderers/vector/VectorRigPreview.tsx` branché dans `RigPreview.tsx`, qui rend le document résolu avec `session.previewValues()`. La `RigSession` (classe dans `src/state/session.ts`, `drivenValues`, `animationFrom`, snapshots, historique, pistes) fonctionne alors sans modification : un document vectoriel paramétré est un rig comme les autres.
...
- Onglet **Controls** de l'inspecteur de l'éditeur : le composant `Inspector` du workbench réutilisé tel quel (il rend `ParameterField` par paramètre, avec `ValueSourceController` et `describeValueSource`), branché sur une `RigSession` créée pour le document courant (même persistance de valeurs que les autres rigs, `src/state/persistence.ts`) ; en tête, un interrupteur **Edit / Tune** : Tune rend le canvas en lecture seule avec les contrôleurs seuls actifs et masque poignées et outils d'édition ; Edit revient à l'éditeur. Un bouton « Add control » crée un paramètre sans liaison (pour les macros et expressions), un menu ⋯ par paramètre (Rename, Move to group, Bindings…, Delete).
...
- En mode Tune sur `/r/:id`, l'inspecteur du workbench affiche Controls, Snapshots, History, Bindings comme pour tout rig, plus un bouton « Edit » qui revient à l'éditeur ; le mode choisi est mémorisé par document. La bibliothèque (`src/library/LibraryPage.tsx`, branche `rig.renderer === 'vector'`) montre une vignette rendue avec les valeurs par défaut.
